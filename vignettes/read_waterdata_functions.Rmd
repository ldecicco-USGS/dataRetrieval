...
Peruse the "Additional Articles" - when we find common issues people have with converting their old workflows, we will try to add articles to clarify new workflows. General questions and bug reports can be reported here: <https://github.com/DOI-USGS/dataRetrieval/issues>
...
## API limits: Avoid site-by-site queries

Even if you get an API token as described above, you could still run into limits on how many queries per hour you are allowed. In the vast VAST majority of these cases, this is caused by using a perfectly good NWIS workflow: querying data site-by-site. Because the NWIS web services did not have any limits on queries per hour, many people created workflows that took advantage of looping through a huge list of sites to get data. 

**SITE-BY-SITE QUERIES SHOULD NOW BE AVOIDED!**

If you have a long list of sites, you can ask for them all at once. Under the hood, the services will return multiple pages of data, split up by the 50,000 limit per page. If we're maxing out our limit, that's 50,000,000 rows of data per hour. 

The one caveat is "continuous" data, in which single site queries are probably still the best bet. There's an upper limit of 3 years allowed to be requested at one time. See [Continuous Data](continuous_pr.html) for some strategies on pulling large amounts of continuous data.
...
The `read_waterdata_combined_meta` function replaces the `whatNWISdata` function. This function combines metadata from `read_waterdata_ts_meta` (time series) and `read_waterdata_field_meta` field measurements collections by site.

Time series metadata:
`r dataRetrieval:::get_description("time-series-metadata")`

Field measurement metadata:
`r dataRetrieval:::get_description("field-measurements-metadata")`


```{r}
ts_available <- read_waterdata_combined_meta(monitoring_location_id = "USGS-01491000",
...
```{r echo=FALSE}
ts_available1 <- ts_available[,c("data_type", "parameter_name", "statistic_id", "begin", "end", "last_modified")]
...
Currently this service only allows up to 3 years of data to be requested at once. If no time is specified in the query, the latest year of data is returned. If you manage large data pipelines, this might sound very restrictive. For more information, see [Continuous Data](continuous_pr.html).
...
The `read_waterdata_channel` accompanies `read_waterdata_field_measurements` to replace `readNWISgwl`.

`r dataRetrieval:::get_description("channel-measurements")`

```{r}
df <- read_waterdata_channel(monitoring_location_id = "USGS-02238500")
```


## Field Measurement Metadata

The `read_waterdata_field_meta` helps to replace the `whatNWISdata` and `whatNWISsites` functions.

`r dataRetrieval:::get_description("field-measurements-metadata")`

```{r eval=FALSE}
df <- read_waterdata_field_meta(monitoring_location_id = "USGS-375907091432201")
```
...
See [USGS Reference Lists](articles/Reference_Lists.html) for more detail.
...
Pre-computed temporal summary statistics can be accessed via the `read_waterdata_stats_por` and `read_waterdata_stats_daterange` functions.
This is also a modern USGS endpoint, but does not follow the same OGC standards as the other `read_waterdata` functions.

### Day-of-year and month-of-year statistics

The `read_waterdata_stats_por` function returns day-of-year and month-of-year statistics.
In the example below we pull the day-of-year arithmetic mean discharge value for site USGS-05428500, limiting the month-day range to include February 27 through March 2.
Note that we've selected a subset of the columns returned in the full output for illustration.

```{r states_por_pull}
por_mean <- read_waterdata_stats_por(
  monitoring_location_id = "USGS-05428500",
  parameter_code = "00060",
  start_date = "02-27",
  end_date = "03-02",
  computation_type = c("arithmetic_mean")
)
```

```{r states_por_table, echo=FALSE}
por_mean |>
  sf::st_drop_geometry() |>
  dplyr::select(
...
The first row in the output below summarizes the average daily discharge value across all February 27ths in the site's period of record.
The last row of the output contains the March month-of-year average (i.e., the average daily value across all Marches in the site's period of record). 
This is returned if the `start_date` to `end_date` range includes the first day of a month (e.g., `03-01`).

### Date range-based statistics

Sometimes you want to compute summary statistics for a specific date range.
The `read_waterdata_stats_daterange` returns daily data statistics for calendar month, calendar year, and water year.
The example below illustrates how to pull data from October 1, 2024 through January 31, 2025.
Again, we've selected a subset of the columns from the full output for illustration.

```{r states_daterange_pull}
daterange_mean <- read_waterdata_stats_daterange(
  monitoring_location_id = "USGS-05428500",
  parameter_code = "00060",
...
```{r states_daterange_table, echo=FALSE}
daterange_mean |>
  sf::st_drop_geometry() |>
  dplyr::select(
    monitoring_location_id,
...
The following notes are pretty subtle, and probably not needed for the vast majority of `dataRetrieval` users. 
