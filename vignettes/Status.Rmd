...
df <- data.frame(
  ...
  New = c(
    ...
    "`read_waterdata_field_measurements`, `read_waterdata_channel`",
    "`read_waterdata`",
...
knitr::kable(
  df,
  col.names = c(
    "WaterServices (legacy) function",
    "Water Data (new) function",
    "Available on (branch name)"
  )
)
```
...
If you manage large data pipelines, you might notice some timeouts or errors indicating your request was too large. This will be especially true for the continuous data. Continuous data requests are only allowed up to 3 years of data in a single request. See [Continuous Data](continuous_pr.html) for more information.
...
For real-time information on WQP service availability, visit the [WQP Status Page](https://rconnect.usgs.gov/wqp-status/). The status page shows the success/failure of a set of test queries to both the legacy and beta WQP services, updated every 10 minutes.
...
A new access point for the National Groundwater Monitoring Network. The original `readNGWMNdata` functions have been broken for some time, the `read_ngwmn` set of functions are now available to access this data.
